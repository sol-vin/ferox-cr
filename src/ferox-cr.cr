...
lib Ferox
  ...
  struct Vertices
    data : StaticArray(Vector2, GEOMETRY_MAX_VERTEX_COUNT)
    count : LibC::Int
  end
  ...
  # Broadphase
  fun create_spatial_hash = frCreateSpatialHash(bounds : Rectangle, cell_size : LibC::Float) : SpatialHash*
  fun release_spatial_hash = frReleaseSpatialHash(hash : SpatialHash*)
  fun add_to_spatial_hash = frAddToSpatialHash(hash : SpatialHash*, rec : Rectangle, value : LibC::Int)
  ...
  fun query_spatial_hash = frQuerySpatialHash(hash : SpatialHash*, rec : Rectangle, queries : LibC::Int**)
  fun get_spatial_hash_bounds = frGetSpatialHashBounds(hash : SpatialHash*) : Rectangle
  fun get_spatial_hash_cell_size = frGetSpatialHashCellSize(hash : SpatialHash*) : LibC::Float
  fun set_spatial_hash_bounds = frSetSpatialHashBounds(hash : SpatialHash*,  bounds : Rectangle)
  fun set_spatial_hash_cell_size = frSetSpatialHashCellSize(hash : SpatialHash*, cell_size : LibC::Float)
  fun compute_spatial_hash_key = frComputeSpatialHashKey(hash : SpatialHash*, v : Vector2) : LibC::Int
  ...
  fun draw_arrow = frDrawArrow(p1 : Vector2, p2 : Vector2, thick : LibC::Float, color : Color)
  ...
  # Dynamics
  fun create_body = frCreateBody(type : BodyType, flags : BodyFlags, p : Vector2) : Body*
  fun create_body_from_shape = frCreateBodyFromShape(type : BodyType, flags : BodyFlags, p : Vector2, s : Shape*) : Body*
  ...
  fun get_body_aabb = frGetBodyAABB(b : Body*) : Rectangle
  fun get_body_user_data = frGetBodyUserData(b : Body*) : Void*
  fun get_local_point = frGetLocalPoint(b : Body*, p : Vector2) : Vector2
  fun get_world_point = frGetWorldPoint(b : Body*, p : Vector2) : Vector2
  ...
  fun set_body_position = frSetBodyPosition(b : Body*, p : Vector2)
  ...
  fun apply_gravity = frApplyGravity(b : Body*, gravity : Vector2)
  fun apply_impulse = frApplyImpulse(b : Body*, impulse : Vector2)
  fun apply_torque_impulse = frApplyTorqueImpulse(b : Body*, p : Vector2, impulse : Vector2)
  ...
  fun get_shape_aabb = frGetShapeAABB(s : Shape*, tx : Transform) : Rectangle
  fun get_circle_radius = frGetCircleRadius(s : Shape*) : LibC::Float
  fun get_rectangle_dimensions = frGetRectangleDimensions(s : Shape*) : Vector2
  ...
  fun set_rectangle_dimensions = frSetRectangleDimensions(s : Shape*, wh : Vector2)
  ...
  fun shape_contains_point = frShapeContainsPoint(s : Shape*, tx : Transform, p : Vector2)
  ...
  # World
  fun create_world = frCreateWorld(gravity : Vector2, bounds : Rectangle) : World*
  ...
  fun get_world_bounds = frGetWorldBounds(world : World*) : Rectangle
  ...
  fun set_world_bounds = frSetWorldBounds(world : World*, bounds : Rectangle)
  ...
  fun query_world_spatial_hash = frQueryWorldSpatialHash(world : World*, bounds : Rectangle, bodies : Body**) : LibC::Int
  ...
  fun rec_pixels_to_meters = frRecPixelsToMeters(rec : Rectangle) : Rectangle
  fun rec_meters_to_pixels = frRecMetersToPixels(rec : Rectangle) : Rectangle
  ...
  fun vec2_cross_product = frVec2CrossProduct(v1 : Vector2, v2 : Vector2) : LibC::Float
  fun vec2_dot_product = frVec2DotProduct(v1 : Vector2, v2 : Vector2) : LibC::Float
  ...
  fun vec2_approx_equals? = frVec2ApproxEquals(v1 : Vector2, v2 : Vector2) : Bool
  ...
  fun vec2_pixels_to_meters = frVec2PixelsToMeters(v : Vector2) : Vector2
  fun vec2_meters_to_pixels = frVec2MetersToPixels(v : Vector2) : Vector2
end